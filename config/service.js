...
Service.getConfig = async function () {
    ...
    let {__return, configRsq: {errMsg, configItemList}} = await DCacheOptPrx.getCacheConfigItemList(option);
...
Service.updateServerConfigItemBatch = async function ({serverConfigList}) {
    ...
    serverConfigList.forEach(item => {
        ...
        option.readFromObject({appName, moduleName, serverName: serverName ? 'DCache.' + serverName: '', nodeName, itemId, configValue, configFlag, lastUser, indexId});
        array.push(option);
    });
